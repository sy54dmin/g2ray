...
if [[ x"${release}" == x"centos" ]]; then
    ...
    if [[ ${os_version} -lt 16 ]]; then
        echo -e "${red}Please use Ubuntu 18 or later! ${plain}\n" && exit 1
    fi
...
install_x-ui() {
    ...
    #echo -e "Please ensure that this port is not used by other services, ${yellow} and ensure that port 54321 has been released ${plain}"
